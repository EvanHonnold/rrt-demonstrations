...
from helpers.drawing import clear_output_dir, save_current_figure, draw_environment, draw_tree, draw_path, output_path
...
def consider_other_potential_parents(new_node, neighbors, tree, obstacles):
    ...
    for neighbor in neighbors:
        if not line_collides_with_obstacles(LineString([new_node, neighbor]), obstacles):
            current_cost = tree.shortest_path_to_root_cost(
                neighbor) + tree.dist(neighbor, new_node)
...
for i in range(6000):
    ...
    if not line_collides_with_obstacles(driving_path, env.obstacles):
        ...
        if MODE == "RRT-star":
            ...
            consider_other_potential_parents(
                new_node, neighbors, tree, env.obstacles)
    ...
    sample_to_goal = euclidean_distance(driving_path.coords[-1], env.goal)
    path_to_goal = euclidean_distance(closest_node_on_tree, env.goal)

    path = tree.shortest_path_from_root(env.goal)
    path_len = path_length(path, distance_function=euclidean_distance)
    print(
        f"pass={i} path_to_goal={path_to_goal:0.2f} sample_to_goal={sample_to_goal:0.2f} path_length={path_len:0.2f}")

    if i % 50 == 0:
        draw_and_snap()  # save every 50th


draw_and_snap()  # save the final position

animation = camera.animate()
animation.save(output_path('gif'), writer='imagemagick')
