...
def plot_polygons(polygons: List[Polygon]) -> None:
    polygon_patches = [patches.Polygon(
        np.array(p.exterior.xy).transpose()) for p in polygons]
...
def save_current_figure() -> None:
    ...
    pyplot.savefig(output_path("pdf"))


def output_path(extension):
    other_file_count = len([name for name in os.listdir('./output/')])
    return f'output/figure_{other_file_count}.{extension}'
